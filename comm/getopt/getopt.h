...
//    - First argument is default option value, then all option indentifiers
//    follow.
//
// int main() {
...
// }
//
// 2) Simple OOP map-based api `getopt class`. Initialization (argc, argv) pair
// required.
//
//    This getopt class is a std::map replacement where key/value are
//    std::string types. Given invokation './app.out --user=me --pass=123 -h'
//    this class delivers not only: map[0] = "./app.out", map[1] = "--user=me",
//    map[2]="--pass=123", map[3]='-h' but also, map["--user"]="me",
//    map["--pass"]="123" and also, map["-h"]=true
//
//    Additional API:
//    - .cmdline() for a print app invokation string
//    - .str() for pretty map printing
//    - .size() number of arguments (equivalent to argc), rather than
//    std::map.size()
//
// int main( int argc, const char **argv ) {
//     getopt args( argc, argv );
//     if( args.has("-h") || args.has("--help") || args.has("-?") || args.size()
//     == 1 ) {
//         std::cout << args["0"] << " [-?|-h|--help] [-v|--version]
//         [--depth=number]" << std::endl; return 0;
//     }
//     if( args.has("-v") || args.has("--version") ) {
//         std::cout << args["0"] << " sample v1.0.0. Compiled on " << __DATE__
//         << std::endl;
//     }
//     if( args.has("--depth") ) {
...
struct getopt : public std::map<std::string, std::string> {
    ...
    getopt(int argc, const char **argv) : super() {
        ...
        for (auto &it : args) {
            ...
            auto size = comm::split(tokens, it, "=");
...
inline T getarg(const T &defaults, const char *argv) {
    static struct getopt map(comm::cmdline());
    return map.has(argv) ? comm::as<T>(map[argv]) : defaults;
}
...
inline const char *getarg(const char *defaults, const char *argv) {
    static struct getopt map(comm::cmdline());
    return map.has(argv) ? comm::as<const char *>(map[argv]) : defaults;
}
